...
(function() {
	lyGrid = (function(params) {
			...
			if(conf.local){
				...
				json='';
					...
					error : function(msg) {
						...
						json = '';
						console.log(msg);
						if(msg.responseText.indexOf("系统登录")!=-1){
		...
		var cBodytb = function(divId,jsonData){
			...
			var  e = json.length;
			if(conf.local){
				pNow = parseInt(_getValueByName(jsonData, conf.pageNum),10);
		...
		var fenyeDiv = function(divid, jsonData) {
			...
			var pageNum = _getValueByName(jsonData, conf.pageNum);
			if(conf.local){
			...
			var pg = pagesIndex(conf.pagecode, pageNum, totalPages);
		...
		var pageBind = function() { // 页数
			...
			if(conf.local){
				returnData.pageNum=parseInt(page,10);
				cBodytb(divid,returnData);
				if (conf.usePage) {// 是否分页
				fenyeDiv(divid, returnData);
			}
		...
		/**
		 * 这是一个分页工具 主要用于显示页码,得到返回来的 开始页码和结束页码 pagecode 要获得记录的开始索引 即 开始页码 pageNum
		 * 当前页 pageCount 总页数
		 *
		 */
		var pagesIndex = function(pagecode, pageNum, pageCount) {
			/*
			 ...
			 * pagesIndex(pagecode, pageNum,totalPages); var
			 * startpage=sten.start; var endpage=sten.end;
			 */
			pagecode = parseInt(pagecode,10);
			pageNum = parseInt(pageNum,10);
			pageCount = parseInt(pageCount,10);
			var startpage = pageNum - (pagecode % 2 == 0 ? pagecode / 2 - 1 : pagecode / 2);
			var endpage = pageNum + pagecode / 2;
			if (startpage < 1) {
				startpage = 1;
