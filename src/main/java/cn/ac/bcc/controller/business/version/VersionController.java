...
public class VersionController extends BaseController<Version> {
    ...
    public String add(MultipartFile file, Version version, HttpServletRequest request){
        ...
        if (!file.isEmpty()) {
            try {
                String tomcatPath = request.getServletContext().getRealPath("/upload//");
                File filepath = new File(tomcatPath + "/" + "version");
                if (!filepath.exists())
                    filepath.mkdirs();
                FileCopyUtils.copy(file.getBytes(), new File(tomcatPath + "/version/" + versionName ));
        ...
        String url = "/upload/version/"+versionName;
        version.setUrl(url);
